...
/// The skeleton driver's digest, re-exported where its callers already
/// look. The byte-hashing core lives in [`blam::hash`] (hoisted there as
/// pure code motion when the qALC fixture commitments needed it from the
/// library side); this checkpoint module keeps only the
/// filesystem-facing [`sha256_16`] fingerprint.
pub use blam::hash::sha256_hex;
