...
//! copy-on-fork at `meas`. Lockstep-tested against `qvm.rs` (the executable
...
//!   qvm's own — one implementation, so amplitudes compare bit-identically.
//! - Fates, budgets, and leaves are qvm's types; `QBudget.beta` means the
...
//! meeting a Prim1/Cnot1/Cnot2 on top), which is exactly qvm's "Err
//! precedes effects, before the value's interior is normalized".

use crate::qvm::{Capacity, ErrKind, Fate, Leaf, Prim, QBudget, Store};
...
impl Pool {
    ...
    /// Import an eval/qvm-side term (tests).
...
enum Val {
    ...
    /// recipes duplicate, matching qvm's substitution semantics exactly.
    Clo(u32, u32),
...
impl QMachine {
    ...
    /// Leaf order matches qvm exactly (outcome-0 first, depth-first).
    pub fn run_into(
        &mut self,
        pool: &mut Pool,
        root: u32,
...
mod tests {
    ...
    fn qvm_leaves(src: &str, order: &[Prim; 5], budget: &QBudget) -> Vec<Leaf> {
        let t = parse_all(src).unwrap();
        qvm::run(qvm::apply_signature(&t, order), budget)
    }
    ...
    fn lockstep_population(max_n: u32, order: &[Prim; 5], ref_budget: &QBudget) {
        ...
        programs.par_iter().for_each_init(
            ...
            |(pool, m), &(enc, len)| {
                ...
                let expect = qvm_leaves(&src, order, ref_budget);
                let mut got = Vec::new();
    ...
    fn lockstep_beta_boundary() {
        ...
        // (qvm performs the β-th contraction, then declares Unknown at the
    ...
    // ---- direct machine vectors (pool-built, independent of qvm) ---------
