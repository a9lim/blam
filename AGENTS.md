...
- Engine config is data, not ambient: on the ladder path (`census`,
  `adjudicate`, `solomonoff`) `args::engine_cfg` resolves flag → environment
  → measured default and passes an `EngineCfg` down. The exceptions are the
  deprecated shims `escalation::normal_form` and `normal_form_spine` (no
  `_with` suffix) — the only environment reads left in the library,
  `OnceLock`-cached, so a mid-run `set_var` is invisible.
  `quantum::certificate`, `q census`'s `--skeleton` column, and `blam slots`
  still call them, so `q skeleton`, `q census`, and `cert search` write the
  variables through `args::apply_engine_env` *before* any compute starts.
  Migrating a caller means switching it to the `_with` entry point, not adding
  a read. `blam slots` has no knob flags at all and answers only to the raw
  environment.
