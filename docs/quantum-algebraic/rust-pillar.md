...
**Status: IMPLEMENTED 2026-08-17 (ratified 2026-08-14), spar-complete on thread
...
**Phase 3 is landed** (2026-08-17: `compiler.rs`, `shadow.rs`, and
`gate2check.rs`; distinct source-gate and dynamic-port types; explicit Gate-2
machine selection; compiler-indexed shortened-log WF; structural admission;
transparency/nonvacuity, exact predecessor/range, Gram, common-cut, runtime,
literal-row, ideal-column, terminal-garbage, selector/mutation, and large-core
checks. `qalc/export_gate2_fixtures.py` pins the complete 917-state / 913-column
mixed carrier, term, certificate, trace, and finals byte-for-byte, plus a
Python-generated byte manifest for all five compiler cases also pinned in
Lean. The
differential caught and corrected an initial fresh-vs-first-recall CNOT frame
epoch drift that carrier counts did not expose.) **Phase 4 is landed**
(2026-08-17: `admission.rs`, the typed `wf.rs` subset and direct RRI check,
embedded frozen-candidate pins, total compiler/Gate-1/conservative selection,
and public exact semantic objects in `semantics.rs`. The 20 embedded
certificate candidates revalidate and remain selected ahead of the checked
no-erasure retry; this is a pinned selector surface, not a port of Python
certificate discovery. Norm and monotone halt mass
hold at every one of the suite's 2,361 transitions; HH, H–NOT′–H, negative,
density, exact-Kraft, fallback-predecessor, effect-free, and public compiled
Gate-2 checks pass; the independent normalizer surface covers 201 pure closed
terms through size 7 and all 105 effect-free `p h t` invocations among them.)
The Phase-4 evidence is an independent Rust battery over the
already-differential transition layers, not a separate semantic-output
Python/Rust fixture family.
`architecture.md`
...
- **Phase 2 — composed machine.** `readback.rs`, BA adapters, typed
  ...
  `U`.
- **Phase 3 — Gate-2 (landed 2026-08-17).** `compiler.rs` + `shadow.rs` +
  `gate2check.rs`.
  ...
  controls; the exact 917-state eight-shard rows. The five term/certificate
  pins come from the Python-generated `compiler_pins.txt` manifest for the
  same `empty1`/`h1`/`t1`/`cx2`/`mixed2` cases pinned by Lean.
- **Phase 4 — admission and semantic objects (landed 2026-08-17).** `admission.rs` +
...
- **Sort-site inventory**: `PyReprKey`'s domain is the two
  ...
  Phase-3 correction, verified against the source and the reachable
  differential where applicable:
  `CP`/`CH` remain in KS and never enter an RS sort. The shadow table inserts
  `c1`/`c2` **frames** into RS. `CGAM` can be captured inside an LP slice and
  thereby enter a frame/key repr by construction; the selected carrier does
  not exhibit that capture, so the Gate-2 corpus pins the nested `CGAM`
  instance synthetically. `GateTag` therefore extends the two sort-domain
  records with `c1`/`c2`.
- Clean compiler carriers do not reach the Gate-2 shadow's typed error arms or
  refused custom-predecessor branches. As with the residual Gate-1 guard
  surface, those arms are review-pinned, not claimed as differential coverage.
