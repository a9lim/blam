...
| Instance identity | **gate-copy address proved; lifecycle injectivity open** (`instance-identity.md`; `kernel.md` §7.5, §9): for the fixed shell, W1 makes each gate log the singleton `(i)` and `g` selects the unique leaf, so `(g,i)` reconstructs the complete gate-copy address and distinct copies cannot alias. General lp uniqueness is false. The alternative raw-WF transition theorem is also false: idempotent `recall` maps an agreeing frame-absent/frame-present pair to one target. The framed clone of the exact `negative` witness is unreachable, and registered finite attacks found no reachable pair. The direct induction nevertheless fails at certified pop with a surviving replay ticket and at cross-fibre phase separation; no-rider + alpha ancestry + cross-phase separation would imply RRI, but the global factors remain open |
