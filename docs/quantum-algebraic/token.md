...
| Item | Status |
|---|---|
| Kernel | **v1.6 — PASS claimed for the h-fragment** (`kernel.md` §13), pending fresh-instance adversarial review: conservation theorem proved (ledger + coloring + transfer) and mechanized at the invariant level (41,272 closed terms, zero failures); terminal chains, gauge-pinned uniqueness (4/256 = predicted orbit), `discover_total`, and the lp invariants all discharged; fences typed and registered (buried-frame, literal gate application, t-gate reservation) |
