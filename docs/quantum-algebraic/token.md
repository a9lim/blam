...
| Instance identity | **ratified; naming discharged, interleaving OPEN** (`kernel.md` §13.4, §13.6): lp-at-log-head proved structurally for signature-abstracted programs; equal-lps-same-copy backed by the swept `(g,i)`-aliasing regression plus the `dup` witness. But `buried-frame` is NOT an unreachable fence: the fresh review's `Q = ((h 0̂) SEL SEL)(h 0̂)` reaches it from init (err mass 1, typed, total) — two live instances interleaving re-seeks is the v1.7 design object |
