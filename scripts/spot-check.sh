...
# rules). Halt counts must be bit-identical to data/census_table.txt at
...
awk -v lo="$MIN" -v hi="$MAX" 'NF>=10 && $1+0>0 && $1>=lo && $1<=hi {print $1,$2,$3,$4,$5,$6,$7,$8}' \
    data/census_table.txt > "$T/canon.cols"
diff "$T/spot.cols" "$T/canon.cols"
echo "spot-check OK: rows $MIN..$MAX bit-identical to data/census_table.txt"
